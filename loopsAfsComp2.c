   ...
   // Load transfer block //
    ...
    for (t = 0; t < cnt_ldd_ts[curr_rep]; t++){
      ...
      if (val > max_val){
        ...
        // Wait for threads in previous rep
        //#pragma omp critical
    ...
    if (ldd_t != t_no){
      ...
      #pragma omp atomic read
      lb = t_itr_end[ldd_t];
      //thread adds itself to loaded thread array
      ldd_tarr[lt] = t_no;
      lt++;
      //omp_set_lock(&(lock_itr_left[t_no]));
    }
    if ((cnt_ldd_ts[curr_rep] > 0) && (lt == 0)){
      //Reset as all threads will leave the loop from now on
      getThread = ldd_tarr;
    }
    else{
      getThread = default_ldd_tarr;
    ...
    #pragma omp atomic read
    cntExtThreads = is_assigned[t_no];

    // Dont go to next rep until all threads in my seg have stopped
    if ((lt == 0) && (cntExtThreads == 0))/*((itr == 0) && (cntPrevRep == 0))*/{
      max_val = 0;
    }

    // Either iters to solve or wait for
    else{
      #pragma omp atomic update
      is_assigned[seg]++;
      omp_set_lock(&(lock_itr_left[t_no]));
    ...
    //#pragma omp critical (printf_lock)
    //{printf("Number of external threads running in my loop %d\n", cntExtThreads);}
    // Uncomment for DEBUG
    //#pragma omp critical (printf_lock)
